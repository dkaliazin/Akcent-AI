...
Кнопка `Заповнити Географія 7` запускает тестовый перенос тем и домашних заданий для Географія 7 класс: читает 2 семестр прошлого года и заполняет 2 семестр текущего года.
Кнопка `Почистити Теми уроків Географія 7` очищает темы уроков и домашние задания только для Географія 7 в текущем 2 семестре.
...
Тестовый перенос можно запустить и без панели:

```bash
npm run fill:geography7
```

Очистку текущего журнала Географія 7 можно запустить без панели:

```bash
npm run clear:geography7
```

По умолчанию используются:

```dotenv
TRANSFER_SUBJECT=Географія
TRANSFER_GRADE=7
TRANSFER_SOURCE_SEMESTER=2024-2025 [2]
TRANSFER_TARGET_SEMESTER=2025-2026 [2]
```
