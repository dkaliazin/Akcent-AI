...
const { clearGeography7Homework } = require('./helpers/journalTransfer');
const {
  clickLoginButton,
  openCabinetLogin,
  openHomePage,
...
async function run() {
  ...
  try {
    console.log('Старт очистки Географія 7');
    ...
    await clearGeography7Homework(browser.page, config);

    console.log('Очистка Географія 7 завершена');
  } catch (error) {
    console.error('Очистка завершилась с ошибкой');
    console.error(error);
    process.exitCode = 1;
  } finally {
    if (!context) {
